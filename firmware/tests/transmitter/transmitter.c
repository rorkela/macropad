...
static void nrf24_send_payload(uint8_t *data, uint8_t len)
{
	...
	for(volatile int i = 0; i < 500; i++) __asm__("nop"); // Small delay (>10us)
...
static void nrf24_init(void)
{
	...
	nrf24_write_multi(NRF24_REG_TX_ADDR, rx_addr, 5);
	...
	nrf24_write_reg(NRF24_REG_CONFIG, NRF24_CONFIG_PWR_UP | NRF24_CONFIG_CRCO_1B | NRF24_CONFIG_PRIM_TX);
...
int main(void)
{
	...
	uint8_t packet[1] = {0xFF};

	while (1) {
		nrf24_send_payload(packet, 1);
		for (volatile unsigned i = 0; i < 10000000; i++) __asm__("nop");
	}
}
