...
static void ssd1306_hline(uint8_t y){
    uint8_t page = y / 8;
    uint8_t bit  = y % 8;
        for (int x = 0; x < 128; x++)
            display_buffer[page * 128 + x] |= 1<<bit;
...
static void update_buffer(macro_layer_t layer){
	ssd1306_clear();
	ssd1306_hline(9);
	ssd1306_string(0,0,layer.layer_name);
	for(int i=0;i<3;i++) {
		for(int j=0;j<3;j++){
			ssd1306_string(i*42,10+j*10,layer.binding_names[j*3+i]);
		}
	}
